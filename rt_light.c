...
/*   Updated: 2022/08/19 16:45:08 by schoe            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3	ray_color(t_rt *unit, t_ray *ray, int depth)
{
	...
	if (ray_try_doing_hit(unit->conf.objects, ray, &hit))
	{
		if (hit.obj->type == OBJ_PLANE)
			//hit.normal = get_texture_normal(get_texture_color(hit, ray, unit->conf, block));
			return(get_texture_color(hit, unit->conf, Checker_board));
		if (hit.obj->type == OBJ_SPHERE)
			hit.normal = get_texture_normal(get_texture_color(hit, unit->conf, F_1000));
